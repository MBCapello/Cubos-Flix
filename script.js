...
function segundaSection() {
  fetch(
    ...
    promise.then(function (element) {
      ...
      highlight__genres.textContent =
        element.genres[0].name +
        ", " +
        element.genres[1].name +
        ", " +
        element.genres[2].name;
...
function eventoModal(element1, element2) {
  element1.addEventListener("click", async () => {
    const promise = await fetch(
      ` https://tmdb-proxy.cubos-academy.workers.dev/3/movie/${element2.id}?language=pt-BR`
    );
...
//função de busca do input testando com Async
...
input.addEventListener("keydown", async (event) => {
  ...
  const promesaBusca = await fetch(
...
btn_theme.addEventListener("click", () => {
  btn_theme.src =
    btn_theme.src === "http://127.0.0.1:5500/cubos-flix/assets/light-mode.svg"
      ? "http://127.0.0.1:5500/cubos-flix/assets/dark-mode.svg"
      : "http://127.0.0.1:5500/cubos-flix/assets/light-mode.svg";

  next.src =
    next.src ===
    "http://127.0.0.1:5500/cubos-flix/assets/seta-direita-preta.svg"
      ? "http://127.0.0.1:5500/cubos-flix/assets/seta-direita-branca.svg"
      : "http://127.0.0.1:5500/cubos-flix/assets/seta-direita-preta.svg";

  prev.src =
    prev.src ===
    "http://127.0.0.1:5500/cubos-flix/assets/seta-esquerda-preta.svg"
      ? "http://127.0.0.1:5500/cubos-flix/assets/seta-esquerda-branca.svg"
      : "http://127.0.0.1:5500/cubos-flix/assets/seta-esquerda-preta.svg";

  const novoTemaClaro =
    body.style.getPropertyValue("--tema-claro") === "#000" ? "#fff" : "#000";
  body.style.setProperty("--tema-claro", novoTemaClaro);
  const novoTemaEscuro =
    body.style.getPropertyValue("--tema-escuro") === "#fff" ? "#000" : "#fff";
